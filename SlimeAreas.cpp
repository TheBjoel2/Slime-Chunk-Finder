...
uint32_t calc(const int32_t chunkX, const int32_t chunkZ)
{
    uint32_t volume = 0;
...
bool test(const uint32_t chunkX, const uint32_t chunkZ)
{
    return calc(static_cast<int32_t>(chunkX)-8, static_cast<int32_t>(chunkZ)-8) > 1500;
}

class SimpleSlimeArea
{
public:
    SimpleSlimeArea(const GlobalSeedMatrix& matrix, const uint32_t matrixX, const uint32_t matrixZ):
        matrix(matrix),
        ...
        currentMatrixZ(matrixZ)
    {
        for(uint32_t j = 0; j < 16; j++)
            for(uint32_t i = 0; i < 16; i++)
            {
                ...
                const bool isWithinCircle = test(i, j);
                //storage[j][i] = isSlimeChunk;
                circleAreaSum+=isSlimeChunk&isWithinCircle;
            }
    }

    uint32_t getAreaSum() const { return circleAreaSum; }

    void shiftToPositiveX()
    {
        const auto area = SimpleSlimeArea(matrix, currentMatrixX+1, currentMatrixZ);
        circleAreaSum = area.circleAreaSum;
        currentMatrixX = currentMatrixX+1;
    }
...
class HardcodedSlimeArea
{
public:
    HardcodedSlimeArea(const GlobalSeedMatrix& matrix, const uint32_t matrixX, const uint32_t matrixZ):
        matrix(matrix),
        currentMatrixX(matrixX),
        currentMatrixZ(matrixZ)
    {
...
private:
    ...
    void calculateNewColumn()
    {
        for(uint32_t j = 0; j < 16; j++)
        {
            const bool isSlimeChunk = Util::isSlimeChunk(matrix, currentMatrixX+16, currentMatrixZ+j);
            storage[j][generateToX] = isSlimeChunk;
        }
        currentMatrixX++;
    }
...
class Simd1SlimeArea
{
public:
    Simd1SlimeArea(const GlobalSeedMatrix& matrix, const uint32_t matrixX, const uint32_t matrixZ):
        matrix(matrix),
        currentMatrixX(matrixX),
        currentMatrixZ(matrixZ)
    {
...
private:
    ...
    void calculateNewColumn()
    {
        ...
        stdx::fixed_size_simd<uint64_t, 16> value([&](const uint32_t j)
        {
        ...
        for(uint32_t j = 0; j < 16; j++)
            storage[j][generateToX] = (JavaRandom::nextIntForSeed(value[j], 10) == 0);
...
class Simd2SlimeArea
{
public:
    Simd2SlimeArea(const GlobalSeedMatrix& matrix, const uint32_t matrixX, const uint32_t matrixZ):
        matrix(matrix),
        currentMatrixX(matrixX),
        currentMatrixZ(matrixZ)
    {
